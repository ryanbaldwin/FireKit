...
class MessageHeaderTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testMessageHeader1NillingPopulatability() {
        do {
            let instance = try runMessageHeader1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.MessageHeader()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test MessageHeader successfully, but threw: \(error)")
        }
    }
    ...
    func testMessageHeaderRealm1(_ instance: FireKit.MessageHeader) {
        ...
        let existing = realm.object(ofType: FireKit.MessageHeader.self, forPrimaryKey: newInst.pk)!
