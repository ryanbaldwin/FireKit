...
class DiagnosticReportTests: XCTestCase, RealmPersistenceTesting {    
	...
	func instantiateFrom(filename: String) throws -> FireKit.DiagnosticReport {
		return instantiateFrom(json: try readJSONFile(filename))
	}
	
	func instantiateFrom(json: FHIRJSON) -> FireKit.DiagnosticReport {
	...
	func testDiagnosticReportRealm1(instance: FireKit.DiagnosticReport) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runDiagnosticReport1(realm.objects(FireKit.DiagnosticReport.self).first!.asJSON())
        ...
        existing = realm.object(ofType: FireKit.DiagnosticReport.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runDiagnosticReport1(existing.asJSON())
	...
	func testDiagnosticReportRealm2(instance: FireKit.DiagnosticReport) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runDiagnosticReport2(realm.objects(FireKit.DiagnosticReport.self).first!.asJSON())
	...
	func testDiagnosticReportRealm3(instance: FireKit.DiagnosticReport) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runDiagnosticReport3(realm.objects(FireKit.DiagnosticReport.self).first!.asJSON())
	...
	func testDiagnosticReportRealm4(instance: FireKit.DiagnosticReport) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runDiagnosticReport4(realm.objects(FireKit.DiagnosticReport.self).first!.asJSON())
	...
	func testDiagnosticReportRealm5(instance: FireKit.DiagnosticReport) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runDiagnosticReport5(realm.objects(FireKit.DiagnosticReport.self).first!.asJSON())
	...
	func testDiagnosticReportRealm6(instance: FireKit.DiagnosticReport) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runDiagnosticReport6(realm.objects(FireKit.DiagnosticReport.self).first!.asJSON())
	...
	func testDiagnosticReportRealm7(instance: FireKit.DiagnosticReport) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runDiagnosticReport7(realm.objects(FireKit.DiagnosticReport.self).first!.asJSON())
	...
	func testDiagnosticReportRealm8(instance: FireKit.DiagnosticReport) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runDiagnosticReport8(realm.objects(FireKit.DiagnosticReport.self).first!.asJSON())
