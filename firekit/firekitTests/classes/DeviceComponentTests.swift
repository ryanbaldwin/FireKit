...
class DeviceComponentTests: XCTestCase, RealmPersistenceTesting {    
	...
	func instantiateFrom(filename: String) throws -> FireKit.DeviceComponent {
		return instantiateFrom(json: try readJSONFile(filename))
	}
	
	func instantiateFrom(json: FHIRJSON) -> FireKit.DeviceComponent {
	...
	func testDeviceComponentRealm1(instance: FireKit.DeviceComponent) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runDeviceComponent1(realm.objects(FireKit.DeviceComponent.self).first!.asJSON())
        ...
        existing = realm.object(ofType: FireKit.DeviceComponent.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runDeviceComponent1(existing.asJSON())
	...
	func testDeviceComponentRealm2(instance: FireKit.DeviceComponent) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runDeviceComponent2(realm.objects(FireKit.DeviceComponent.self).first!.asJSON())
