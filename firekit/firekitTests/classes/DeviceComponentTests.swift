...
class DeviceComponentTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testDeviceComponent1NillingPopulatability() {
        do {
            let instance = try runDeviceComponent1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.DeviceComponent()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test DeviceComponent successfully, but threw: \(error)")
        }
    }
    ...
    func testDeviceComponentRealm1(_ instance: FireKit.DeviceComponent) {
        ...
        let existing = realm.object(ofType: FireKit.DeviceComponent.self, forPrimaryKey: newInst.pk)!
    ...
    func testDeviceComponent2NillingPopulatability() {
        do {
            let instance = try runDeviceComponent2()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.DeviceComponent()) }
