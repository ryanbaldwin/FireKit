...
class ConceptMapTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testConceptMap1NillingPopulatability() {
        do {
            let instance = try runConceptMap1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.ConceptMap()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test ConceptMap successfully, but threw: \(error)")
        }
    }
    ...
    func testConceptMap2NillingPopulatability() {
        do {
            let instance = try runConceptMap2()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.ConceptMap()) }
