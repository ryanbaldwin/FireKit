...
class QuestionnaireTests: XCTestCase, RealmPersistenceTesting {    
	...
	func instantiateFrom(filename: String) throws -> FireKit.Questionnaire {
		return instantiateFrom(json: try readJSONFile(filename))
	}
	
	func instantiateFrom(json: FHIRJSON) -> FireKit.Questionnaire {
	...
	func testQuestionnaireRealm1(instance: FireKit.Questionnaire) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runQuestionnaire1(realm.objects(FireKit.Questionnaire.self).first!.asJSON())
	...
	func testQuestionnaireRealm2(instance: FireKit.Questionnaire) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runQuestionnaire2(realm.objects(FireKit.Questionnaire.self).first!.asJSON())
	...
	func testQuestionnaireRealm3(instance: FireKit.Questionnaire) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runQuestionnaire3(realm.objects(FireKit.Questionnaire.self).first!.asJSON())
	...
	func testQuestionnaireRealm4(instance: FireKit.Questionnaire) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runQuestionnaire4(realm.objects(FireKit.Questionnaire.self).first!.asJSON())
