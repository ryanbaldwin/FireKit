...
class DiagnosticOrderTests: XCTestCase, RealmPersistenceTesting {    
	...
	func instantiateFrom(filename: String) throws -> FireKit.DiagnosticOrder {
		return instantiateFrom(json: try readJSONFile(filename))
	}
	
	func instantiateFrom(json: FHIRJSON) -> FireKit.DiagnosticOrder {
    ...
    func testDiagnosticOrder1RealmPK() {        
        do {
            ...
            XCTAssertTrue(false, "Must instantiate and test DiagnosticOrder's PKs, but threw: \(error)")
        }
    }

	func testDiagnosticOrderRealm1(instance: FireKit.DiagnosticOrder) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runDiagnosticOrder1(realm.objects(FireKit.DiagnosticOrder.self).first!.asJSON())
        ...
        existing = realm.object(ofType: FireKit.DiagnosticOrder.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runDiagnosticOrder1(existing.asJSON())
	...
	func testDiagnosticOrderRealm2(instance: FireKit.DiagnosticOrder) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runDiagnosticOrder2(realm.objects(FireKit.DiagnosticOrder.self).first!.asJSON())
	...
	func testDiagnosticOrderRealm3(instance: FireKit.DiagnosticOrder) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runDiagnosticOrder3(realm.objects(FireKit.DiagnosticOrder.self).first!.asJSON())
