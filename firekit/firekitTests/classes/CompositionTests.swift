...
class CompositionTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func inflateFrom(data: Data) throws -> FireKit.Composition {
        // print("Inflating FireKit.Composition from data: \(data)")
    ...
    func testComposition1Copying() {
        do {
            ...
            XCTAssertTrue(false, "Must copy and test Composition successfully, but threw: \(error)")
        }
    }

    func testComposition1Populatability() {
        do {
            let instance = try runComposition1()
            let copy = FireKit.Composition()
            copy.populate(from: instance)
            XCTAssertNotEqual(instance.pk, copy.pk)
            try runComposition1(try JSONEncoder().encode(copy))
        }
        catch let error {
            XCTAssertTrue(false, "Must populate an test Composition successfully, but threw: \(error)")
        }
    }

    func testComposition1NillingPopulatability() {
        do {
            let instance = try runComposition1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.Composition()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test Composition successfully, but threw: \(error)")
        }
    }
    ...
    func testCompositionRealm1(_ instance: FireKit.Composition) {
        ...
        let existing = realm.object(ofType: FireKit.Composition.self, forPrimaryKey: newInst.pk)!
