...
class ImmunizationTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testImmunization1Copying() {
        do {
            ...
            XCTAssertTrue(false, "Must copy and test Immunization successfully, but threw: \(error)")
        }
    }

    func testImmunization1Populatability() {
        do {
            let instance = try runImmunization1()
            let copy = FireKit.Immunization()
    ...
    func testImmunizationRealm1(_ instance: FireKit.Immunization) {
        ...
        let existing = realm.object(ofType: FireKit.Immunization.self, forPrimaryKey: newInst.pk)!
    ...
    func testImmunization2Populatability() {
        do {
            let instance = try runImmunization2()
            let copy = FireKit.Immunization()
            copy.populate(from: instance)
