...
class ImmunizationTests: XCTestCase, RealmPersistenceTesting {    
	...
	func instantiateFrom(filename: String) throws -> FireKit.Immunization {
		return instantiateFrom(json: try readJSONFile(filename))
	}
	
	func instantiateFrom(json: FHIRJSON) -> FireKit.Immunization {
	...
	func testImmunizationRealm1(instance: FireKit.Immunization) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runImmunization1(realm.objects(FireKit.Immunization.self).first!.asJSON())
	...
	func testImmunizationRealm2(instance: FireKit.Immunization) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runImmunization2(realm.objects(FireKit.Immunization.self).first!.asJSON())
