...
class DeviceTests: XCTestCase, RealmPersistenceTesting {    
	...
	func instantiateFrom(filename: String) throws -> FireKit.Device {
		return instantiateFrom(json: try readJSONFile(filename))
	}
	
	func instantiateFrom(json: FHIRJSON) -> FireKit.Device {
	...
	func testDeviceRealm1(instance: FireKit.Device) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runDevice1(realm.objects(FireKit.Device.self).first!.asJSON())
        ...
        existing = realm.object(ofType: FireKit.Device.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runDevice1(existing.asJSON())
	...
	func testDeviceRealm2(instance: FireKit.Device) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runDevice2(realm.objects(FireKit.Device.self).first!.asJSON())
	...
	func testDeviceRealm3(instance: FireKit.Device) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runDevice3(realm.objects(FireKit.Device.self).first!.asJSON())
	...
	func testDeviceRealm4(instance: FireKit.Device) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runDevice4(realm.objects(FireKit.Device.self).first!.asJSON())
	...
	func testDeviceRealm5(instance: FireKit.Device) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runDevice5(realm.objects(FireKit.Device.self).first!.asJSON())
