...
class ImagingObjectSelectionTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testImagingObjectSelection1NillingPopulatability() {
        do {
            let instance = try runImagingObjectSelection1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.ImagingObjectSelection()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test ImagingObjectSelection successfully, but threw: \(error)")
        }
    }
