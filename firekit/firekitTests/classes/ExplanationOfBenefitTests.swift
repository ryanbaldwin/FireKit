...
class ExplanationOfBenefitTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testExplanationOfBenefit1NillingPopulatability() {
        do {
            let instance = try runExplanationOfBenefit1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.ExplanationOfBenefit()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test ExplanationOfBenefit successfully, but threw: \(error)")
        }
    }
