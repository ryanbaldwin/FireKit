...
class DocumentReferenceTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testDocumentReference1NillingPopulatability() {
        do {
            let instance = try runDocumentReference1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.DocumentReference()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test DocumentReference successfully, but threw: \(error)")
        }
    }
