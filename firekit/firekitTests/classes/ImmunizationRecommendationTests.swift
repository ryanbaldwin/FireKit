...
class ImmunizationRecommendationTests: XCTestCase, RealmPersistenceTesting {    
	...
	func instantiateFrom(filename: String) throws -> FireKit.ImmunizationRecommendation {
		return instantiateFrom(json: try readJSONFile(filename))
	}
	
	func instantiateFrom(json: FHIRJSON) -> FireKit.ImmunizationRecommendation {
	...
	func testImmunizationRecommendationRealm1(instance: FireKit.ImmunizationRecommendation) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runImmunizationRecommendation1(realm.objects(FireKit.ImmunizationRecommendation.self).first!.asJSON())
        ...
        var existing = realm.object(ofType: FireKit.ImmunizationRecommendation.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runImmunizationRecommendation1(existing.asJSON())
        
        // second time updating it will overwrite values of child resources/elements, but maintain keys
        // TODO: Find a way to actually test this instead of breakpoints and eyeballing it.
        existing = realm.object(ofType: FireKit.ImmunizationRecommendation.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runImmunizationRecommendation1(existing.asJSON())
