...
class ProcedureRequestTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testProcedureRequest1NillingPopulatability() {
        do {
            let instance = try runProcedureRequest1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.ProcedureRequest()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test ProcedureRequest successfully, but threw: \(error)")
        }
    }
