...
class CommunicationRequestTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testCommunicationRequest1NillingPopulatability() {
        do {
            let instance = try runCommunicationRequest1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.CommunicationRequest()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test CommunicationRequest successfully, but threw: \(error)")
        }
    }
    ...
    func testCommunicationRequestRealm1(_ instance: FireKit.CommunicationRequest) {
        ...
        let existing = realm.object(ofType: FireKit.CommunicationRequest.self, forPrimaryKey: newInst.pk)!
