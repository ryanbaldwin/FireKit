...
class ProcessResponseTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testProcessResponse1NillingPopulatability() {
        do {
            let instance = try runProcessResponse1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.ProcessResponse()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test ProcessResponse successfully, but threw: \(error)")
        }
    }
