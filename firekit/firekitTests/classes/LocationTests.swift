...
class LocationTests: XCTestCase, RealmPersistenceTesting {    
	...
	func instantiateFrom(filename: String) throws -> FireKit.Location {
		return instantiateFrom(json: try readJSONFile(filename))
	}
	
	func instantiateFrom(json: FHIRJSON) -> FireKit.Location {
	...
	func testLocationRealm1(instance: FireKit.Location) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runLocation1(realm.objects(FireKit.Location.self).first!.asJSON())
	...
	func testLocationRealm2(instance: FireKit.Location) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runLocation2(realm.objects(FireKit.Location.self).first!.asJSON())
	...
	func testLocationRealm3(instance: FireKit.Location) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runLocation3(realm.objects(FireKit.Location.self).first!.asJSON())
	...
	func testLocationRealm4(instance: FireKit.Location) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runLocation4(realm.objects(FireKit.Location.self).first!.asJSON())
	...
	func testLocationRealm5(instance: FireKit.Location) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runLocation5(realm.objects(FireKit.Location.self).first!.asJSON())
	...
	func testLocationRealm6(instance: FireKit.Location) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runLocation6(realm.objects(FireKit.Location.self).first!.asJSON())
