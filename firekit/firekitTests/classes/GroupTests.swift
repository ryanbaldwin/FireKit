...
class GroupTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testGroup1NillingPopulatability() {
        do {
            let instance = try runGroup1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.Group()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test Group successfully, but threw: \(error)")
        }
    }
    ...
    func testGroup2NillingPopulatability() {
        do {
            let instance = try runGroup2()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.Group()) }
