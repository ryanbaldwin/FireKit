...
class DocumentManifestTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testDocumentManifest1NillingPopulatability() {
        do {
            let instance = try runDocumentManifest1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.DocumentManifest()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test DocumentManifest successfully, but threw: \(error)")
        }
    }
    ...
    func testDocumentManifestRealm1(_ instance: FireKit.DocumentManifest) {
        ...
        let existing = realm.object(ofType: FireKit.DocumentManifest.self, forPrimaryKey: newInst.pk)!
