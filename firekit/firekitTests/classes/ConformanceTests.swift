...
class ConformanceTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func inflateFrom(filename: String) throws -> FireKit.Conformance {
        return try inflateFrom(data: try readJSONFile(filename))
    }
    
    func inflateFrom(data: Data) throws -> FireKit.Conformance {
        // print("Inflating FireKit.Conformance from data: \(data)")
    ...
    func testConformance1() {   
        var instance: FireKit.Conformance?
        do {
            instance = try runConformance1()
            try runConformance1(try JSONEncoder().encode(instance!))    
        }
        catch let error {
    ...
    func testConformance1Copying() {
        do {
            let instance = try runConformance1()
            let copy = instance.copy() as? FireKit.Conformance
            XCTAssertNotNil(copy)
            XCTAssertNotEqual(instance.pk, copy?.pk)
            try runConformance1(try JSONEncoder().encode(copy!))
        } catch let error {
            XCTAssertTrue(false, "Must copy and test Conformance successfully, but threw: \(error)")
        }
    }

    func testConformance1Populatability() {
        do {
            let instance = try runConformance1()
            let copy = FireKit.Conformance()
            copy.populate(from: instance)
            XCTAssertNotEqual(instance.pk, copy.pk)
            try runConformance1(try JSONEncoder().encode(copy))
        }
        catch let error {
            XCTAssertTrue(false, "Must populate an test Conformance successfully, but threw: \(error)")
        }
    }

    func testConformance1NillingPopulatability() {
        do {
            let instance = try runConformance1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.Conformance()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test Conformance successfully, but threw: \(error)")
        }
    }
    ...
    func testConformanceRealm1(_ instance: FireKit.Conformance) {
        ...
        try! realm.write { realm.add(instance) }
        try! runConformance1(JSONEncoder().encode(realm.objects(FireKit.Conformance.self).first!))
        
        // ensure we can update it.
        try! realm.write { instance.implicitRules = "Rule #1" }
        XCTAssertEqual(1, realm.objects(FireKit.Conformance.self).count)
        XCTAssertEqual("Rule #1", realm.objects(FireKit.Conformance.self).first!.implicitRules)
        
        // create a new instance with default key, save it, then populate it from instance JSON. 
        // PK should persist and not be overwritten.
        let newInst = FireKit.Conformance()
        try! realm.write { realm.add(newInst) }
        
        // first time updating it should inflate children resources/elements which don't exist
        let existing = realm.object(ofType: FireKit.Conformance.self, forPrimaryKey: newInst.pk)!
        
        try! realm.write { realm.delete(instance) }        
        XCTAssertEqual(1, realm.objects(FireKit.Conformance.self).count)

        try! realm.write { realm.delete(existing) }
        XCTAssertEqual(0, realm.objects(FireKit.Conformance.self).count)
    }
    
    @discardableResult
    func runConformance1(_ data: Data? = nil) throws -> FireKit.Conformance {
        let inst = (data != nil) ? try inflateFrom(data: data!) : try inflateFrom(filename: "conformance-example.json")
    ...
    func testConformance2() {   
        ...
        do {
            ...
            try runConformance2(try JSONEncoder().encode(instance!))    
        }
        catch let error {
            XCTAssertTrue(false, "Must instantiate and test Conformance successfully, but threw: \(error)")
        }
    ...
    func testConformance2Copying() {
        do {
            let instance = try runConformance2()
            let copy = instance.copy() as? FireKit.Conformance
            XCTAssertNotNil(copy)
            XCTAssertNotEqual(instance.pk, copy?.pk)
            try runConformance2(try JSONEncoder().encode(copy!))
        } catch let error {
            XCTAssertTrue(false, "Must copy and test Conformance successfully, but threw: \(error)")
        }
    }

    func testConformance2Populatability() {
        do {
            let instance = try runConformance2()
            let copy = FireKit.Conformance()
            copy.populate(from: instance)
    ...
    func testConformance2NillingPopulatability() {
        do {
            let instance = try runConformance2()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.Conformance()) }
    ...
    func testConformanceRealm2(_ instance: FireKit.Conformance) {
        ...
        // and ensure it passes the all the same tests.
        try! realm.write { realm.add(instance) }
        try! runConformance2(JSONEncoder().encode(realm.objects(FireKit.Conformance.self).first!))
    ...
    func runConformance2(_ data: Data? = nil) throws -> FireKit.Conformance {
        let inst = (data != nil) ? try inflateFrom(data: data!) : try inflateFrom(filename: "conformance-phr-example.json")
