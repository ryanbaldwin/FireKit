...
class DeviceMetricTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testDeviceMetric1NillingPopulatability() {
        do {
            let instance = try runDeviceMetric1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.DeviceMetric()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test DeviceMetric successfully, but threw: \(error)")
        }
    }
