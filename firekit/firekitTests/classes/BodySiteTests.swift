...
class BodySiteTests: XCTestCase, RealmPersistenceTesting {    
	...
	func instantiateFrom(_ filename: String) throws -> FireKit.BodySite {
		return instantiateFrom(try readJSONFile(filename))
	}
	
	func instantiateFrom(_ json: FHIRJSON) -> FireKit.BodySite {
		let instance = FireKit.BodySite(json: json)
	...
	func testBodySiteRealm1(_ instance: FireKit.BodySite) {
        ...
        try! runBodySite1(realm.objects(FireKit.BodySite.self).first!.asJSON())
        ...
        var existing = realm.object(ofType: FireKit.BodySite.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runBodySite1(existing.asJSON())
        
        // second time updating it will overwrite values of child resources/elements, but maintain keys
        // TODO: Find a way to actually test this instead of breakpoints and eyeballing it.
        existing = realm.object(ofType: FireKit.BodySite.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runBodySite1(existing.asJSON())
	...
	@discardableResult
	func runBodySite1(_ json: FHIRJSON? = nil) throws -> FireKit.BodySite {
		let inst = (nil != json) ? instantiateFrom(json!) : try instantiateFrom("bodysite-example.json")
