...
class FamilyMemberHistoryTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testFamilyMemberHistory1NillingPopulatability() {
        do {
            let instance = try runFamilyMemberHistory1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.FamilyMemberHistory()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test FamilyMemberHistory successfully, but threw: \(error)")
        }
    }
    ...
    func testFamilyMemberHistory2NillingPopulatability() {
        do {
            let instance = try runFamilyMemberHistory2()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.FamilyMemberHistory()) }
