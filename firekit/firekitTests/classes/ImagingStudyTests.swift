...
class ImagingStudyTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testImagingStudy1Copying() {
        do {
            ...
            XCTAssertTrue(false, "Must copy and test ImagingStudy successfully, but threw: \(error)")
        }
    }

    func testImagingStudy1Populatability() {
        do {
            let instance = try runImagingStudy1()
            let copy = FireKit.ImagingStudy()
            copy.populate(from: instance)
            XCTAssertNotEqual(instance.pk, copy.pk)
            try runImagingStudy1(try JSONEncoder().encode(copy))
        }
        catch let error {
            XCTAssertTrue(false, "Must populate an test ImagingStudy successfully, but threw: \(error)")
        }
    }

    func testImagingStudy1NillingPopulatability() {
        do {
            let instance = try runImagingStudy1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.ImagingStudy()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test ImagingStudy successfully, but threw: \(error)")
        }
    }
    ...
    func testImagingStudyRealm1(_ instance: FireKit.ImagingStudy) {
        ...
        let existing = realm.object(ofType: FireKit.ImagingStudy.self, forPrimaryKey: newInst.pk)!
