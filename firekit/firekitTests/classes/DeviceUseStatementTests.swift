...
class DeviceUseStatementTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testDeviceUseStatement1NillingPopulatability() {
        do {
            let instance = try runDeviceUseStatement1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.DeviceUseStatement()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test DeviceUseStatement successfully, but threw: \(error)")
        }
    }
    ...
    func testDeviceUseStatementRealm1(_ instance: FireKit.DeviceUseStatement) {
        ...
        let existing = realm.object(ofType: FireKit.DeviceUseStatement.self, forPrimaryKey: newInst.pk)!
