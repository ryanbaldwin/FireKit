...
class TestScriptTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func inflateFrom(data: Data) throws -> FireKit.TestScript {
        // print("Inflating FireKit.TestScript from data: \(data)")
    ...
    func testTestScript1Copying() {
        do {
            ...
            XCTAssertTrue(false, "Must copy and test TestScript successfully, but threw: \(error)")
        }
    }

    func testTestScript1Populatability() {
        do {
            let instance = try runTestScript1()
            let copy = FireKit.TestScript()
            copy.populate(from: instance)
            XCTAssertNotEqual(instance.pk, copy.pk)
            try runTestScript1(try JSONEncoder().encode(copy))
        }
        catch let error {
            XCTAssertTrue(false, "Must populate an test TestScript successfully, but threw: \(error)")
        }
    }

    func testTestScript1NillingPopulatability() {
        do {
            let instance = try runTestScript1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.TestScript()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test TestScript successfully, but threw: \(error)")
        }
    }
    ...
    func testTestScriptRealm1(_ instance: FireKit.TestScript) {
        ...
        let existing = realm.object(ofType: FireKit.TestScript.self, forPrimaryKey: newInst.pk)!
    ...
    func testTestScript2Populatability() {
        do {
            let instance = try runTestScript2()
            let copy = FireKit.TestScript()
            copy.populate(from: instance)
    ...
    func testTestScript2NillingPopulatability() {
        do {
            let instance = try runTestScript2()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.TestScript()) }
