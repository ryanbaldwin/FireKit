...
class BinaryTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testBinary1RealmPK() {        
        do {
            ...
            XCTAssertTrue(false, "Must instantiate and test Binary's PKs, but threw: \(error)")
        }
    }
	...
	func testBinaryRealm1(_ instance: FireKit.Binary) {
        ...
        try! runBinary1(realm.objects(FireKit.Binary.self).first!.asJSON())
        ...
        var existing = realm.object(ofType: FireKit.Binary.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runBinary1(existing.asJSON())
        
        // second time updating it will overwrite values of child resources/elements, but maintain keys
        // TODO: Find a way to actually test this instead of breakpoints and eyeballing it.
        existing = realm.object(ofType: FireKit.Binary.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runBinary1(existing.asJSON())
