...
class ReferralRequestTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testReferralRequest1NillingPopulatability() {
        do {
            let instance = try runReferralRequest1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.ReferralRequest()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test ReferralRequest successfully, but threw: \(error)")
        }
    }
