...
class ClaimResponseTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testClaimResponse1NillingPopulatability() {
        do {
            let instance = try runClaimResponse1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.ClaimResponse()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test ClaimResponse successfully, but threw: \(error)")
        }
    }
    ...
    func testClaimResponseRealm1(_ instance: FireKit.ClaimResponse) {
        ...
        let existing = realm.object(ofType: FireKit.ClaimResponse.self, forPrimaryKey: newInst.pk)!
