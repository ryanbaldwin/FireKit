...
class EpisodeOfCareTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testEpisodeOfCare1NillingPopulatability() {
        do {
            let instance = try runEpisodeOfCare1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.EpisodeOfCare()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test EpisodeOfCare successfully, but threw: \(error)")
        }
    }
