...
class AppointmentTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testAppointment1Copying() {
        do {
            ...
            XCTAssertTrue(false, "Must copy and test Appointment successfully, but threw: \(error)")
        }
    }

    func testAppointment1Populatability() {
        do {
            let instance = try runAppointment1()
            let copy = FireKit.Appointment()
            copy.populate(from: instance)
            XCTAssertNotEqual(instance.pk, copy.pk)
            try runAppointment1(try JSONEncoder().encode(copy))
        }
        catch let error {
            XCTAssertTrue(false, "Must populate an test Appointment successfully, but threw: \(error)")
        }
    }

    func testAppointment1NillingPopulatability() {
        do {
            let instance = try runAppointment1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.Appointment()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test Appointment successfully, but threw: \(error)")
        }
    }
    ...
    func testAppointmentRealm1(_ instance: FireKit.Appointment) {
        ...
        let existing = realm.object(ofType: FireKit.Appointment.self, forPrimaryKey: newInst.pk)!
    ...
    func testAppointment2Populatability() {
        do {
            let instance = try runAppointment2()
            let copy = FireKit.Appointment()
            copy.populate(from: instance)
    ...
    func testAppointment2NillingPopulatability() {
        do {
            let instance = try runAppointment2()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.Appointment()) }
    ...
    func testAppointment3Populatability() {
        do {
            let instance = try runAppointment3()
            let copy = FireKit.Appointment()
            copy.populate(from: instance)
    ...
    func testAppointment3NillingPopulatability() {
        do {
            let instance = try runAppointment3()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.Appointment()) }
