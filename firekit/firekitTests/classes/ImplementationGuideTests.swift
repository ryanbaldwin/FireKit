...
class ImplementationGuideTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testImplementationGuide1NillingPopulatability() {
        do {
            let instance = try runImplementationGuide1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.ImplementationGuide()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test ImplementationGuide successfully, but threw: \(error)")
        }
    }
    ...
    func testImplementationGuideRealm1(_ instance: FireKit.ImplementationGuide) {
        ...
        let existing = realm.object(ofType: FireKit.ImplementationGuide.self, forPrimaryKey: newInst.pk)!
