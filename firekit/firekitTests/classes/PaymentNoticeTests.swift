...
class PaymentNoticeTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testPaymentNotice1NillingPopulatability() {
        do {
            let instance = try runPaymentNotice1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.PaymentNotice()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test PaymentNotice successfully, but threw: \(error)")
        }
    }
    ...
    func testPaymentNoticeRealm1(_ instance: FireKit.PaymentNotice) {
        ...
        let existing = realm.object(ofType: FireKit.PaymentNotice.self, forPrimaryKey: newInst.pk)!
