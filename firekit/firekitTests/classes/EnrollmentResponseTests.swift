...
class EnrollmentResponseTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func testEnrollmentResponse1NillingPopulatability() {
        do {
            let instance = try runEnrollmentResponse1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.EnrollmentResponse()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test EnrollmentResponse successfully, but threw: \(error)")
        }
    }
    ...
    func testEnrollmentResponseRealm1(_ instance: FireKit.EnrollmentResponse) {
        ...
        let existing = realm.object(ofType: FireKit.EnrollmentResponse.self, forPrimaryKey: newInst.pk)!
