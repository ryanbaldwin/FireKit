...
class EnrollmentResponseTests: XCTestCase, RealmPersistenceTesting {    
	...
	func instantiateFrom(filename: String) throws -> FireKit.EnrollmentResponse {
		return instantiateFrom(json: try readJSONFile(filename))
	}
	
	func instantiateFrom(json: FHIRJSON) -> FireKit.EnrollmentResponse {
	...
	func testEnrollmentResponseRealm1(instance: FireKit.EnrollmentResponse) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runEnrollmentResponse1(realm.objects(FireKit.EnrollmentResponse.self).first!.asJSON())
