...
class ValueSetTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func inflateFrom(data: Data) throws -> FireKit.ValueSet {
        // print("Inflating FireKit.ValueSet from data: \(data)")
    ...
    func testValueSet1Copying() {
        do {
            ...
            XCTAssertTrue(false, "Must copy and test ValueSet successfully, but threw: \(error)")
        }
    }

    func testValueSet1Populatability() {
        do {
            let instance = try runValueSet1()
            let copy = FireKit.ValueSet()
    ...
    func testValueSetRealm1(_ instance: FireKit.ValueSet) {
        ...
        let existing = realm.object(ofType: FireKit.ValueSet.self, forPrimaryKey: newInst.pk)!
    ...
    func testValueSet2Populatability() {
        do {
            let instance = try runValueSet2()
            let copy = FireKit.ValueSet()
            copy.populate(from: instance)
    ...
    func testValueSet3Populatability() {
        do {
            let instance = try runValueSet3()
            let copy = FireKit.ValueSet()
            copy.populate(from: instance)
    ...
    func testValueSet4Populatability() {
        do {
            let instance = try runValueSet4()
            let copy = FireKit.ValueSet()
            copy.populate(from: instance)
    ...
    func testValueSet5Populatability() {
        do {
            let instance = try runValueSet5()
            let copy = FireKit.ValueSet()
            copy.populate(from: instance)
    ...
    func testValueSet6Populatability() {
        do {
            let instance = try runValueSet6()
            let copy = FireKit.ValueSet()
            copy.populate(from: instance)
