...
class StructureDefinitionTests: XCTestCase, RealmPersistenceTesting {    
    ...
    func inflateFrom(filename: String) throws -> FireKit.StructureDefinition {
        return try inflateFrom(data: try readJSONFile(filename))
    }
    
    func inflateFrom(data: Data) throws -> FireKit.StructureDefinition {
        // print("Inflating FireKit.StructureDefinition from data: \(data)")
    ...
    func testStructureDefinition1() {   
        var instance: FireKit.StructureDefinition?
        do {
            instance = try runStructureDefinition1()
            try runStructureDefinition1(try JSONEncoder().encode(instance!))    
        }
        catch let error {
    ...
    func testStructureDefinition1Copying() {
        do {
            let instance = try runStructureDefinition1()
            let copy = instance.copy() as? FireKit.StructureDefinition
            XCTAssertNotNil(copy)
            XCTAssertNotEqual(instance.pk, copy?.pk)
            try runStructureDefinition1(try JSONEncoder().encode(copy!))
        } catch let error {
            XCTAssertTrue(false, "Must copy and test StructureDefinition successfully, but threw: \(error)")
        }
    }

    func testStructureDefinition1Populatability() {
        do {
            let instance = try runStructureDefinition1()
            let copy = FireKit.StructureDefinition()
            copy.populate(from: instance)
            XCTAssertNotEqual(instance.pk, copy.pk)
            try runStructureDefinition1(try JSONEncoder().encode(copy))
        }
        catch let error {
            XCTAssertTrue(false, "Must populate an test StructureDefinition successfully, but threw: \(error)")
        }
    }

    func testStructureDefinition1NillingPopulatability() {
        do {
            let instance = try runStructureDefinition1()
            try! realm.write { realm.add(instance) }
            try! realm.write { instance.populate(from: FireKit.StructureDefinition()) }
        } catch let error {
            XCTAssertTrue(false, "Must populate a test StructureDefinition successfully, but threw: \(error)")
        }
    }
    ...
    func testStructureDefinitionRealm1(_ instance: FireKit.StructureDefinition) {
        ...
        try! realm.write { realm.add(instance) }
        try! runStructureDefinition1(JSONEncoder().encode(realm.objects(FireKit.StructureDefinition.self).first!))
        
        // ensure we can update it.
        try! realm.write { instance.implicitRules = "Rule #1" }
        XCTAssertEqual(1, realm.objects(FireKit.StructureDefinition.self).count)
        XCTAssertEqual("Rule #1", realm.objects(FireKit.StructureDefinition.self).first!.implicitRules)
        
        // create a new instance with default key, save it, then populate it from instance JSON. 
        // PK should persist and not be overwritten.
        let newInst = FireKit.StructureDefinition()
        try! realm.write { realm.add(newInst) }
        
        // first time updating it should inflate children resources/elements which don't exist
        let existing = realm.object(ofType: FireKit.StructureDefinition.self, forPrimaryKey: newInst.pk)!
        
        try! realm.write { realm.delete(instance) }        
        XCTAssertEqual(1, realm.objects(FireKit.StructureDefinition.self).count)

        try! realm.write { realm.delete(existing) }
        XCTAssertEqual(0, realm.objects(FireKit.StructureDefinition.self).count)
    }
    
    @discardableResult
    func runStructureDefinition1(_ data: Data? = nil) throws -> FireKit.StructureDefinition {
        let inst = (data != nil) ? try inflateFrom(data: data!) : try inflateFrom(filename: "structuredefinition-example.json")
