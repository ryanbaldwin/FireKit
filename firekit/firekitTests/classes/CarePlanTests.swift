...
class CarePlanTests: XCTestCase, RealmPersistenceTesting {    
	...
	func instantiateFrom(filename: String) throws -> FireKit.CarePlan {
		return instantiateFrom(json: try readJSONFile(filename))
	}
	
	func instantiateFrom(json: FHIRJSON) -> FireKit.CarePlan {
	...
	func testCarePlanRealm1(instance: FireKit.CarePlan) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runCarePlan1(realm.objects(FireKit.CarePlan.self).first!.asJSON())
        ...
        var existing = realm.object(ofType: FireKit.CarePlan.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runCarePlan1(existing.asJSON())
        
        // second time updating it will overwrite values of child resources/elements, but maintain keys
        // TODO: Find a way to actually test this instead of breakpoints and eyeballing it.
        existing = realm.object(ofType: FireKit.CarePlan.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runCarePlan1(existing.asJSON())
	...
	func testCarePlanRealm2(instance: FireKit.CarePlan) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runCarePlan2(realm.objects(FireKit.CarePlan.self).first!.asJSON())
	...
	func testCarePlanRealm3(instance: FireKit.CarePlan) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runCarePlan3(realm.objects(FireKit.CarePlan.self).first!.asJSON())
	...
	func testCarePlanRealm4(instance: FireKit.CarePlan) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runCarePlan4(realm.objects(FireKit.CarePlan.self).first!.asJSON())
	...
	func testCarePlanRealm5(instance: FireKit.CarePlan) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runCarePlan5(realm.objects(FireKit.CarePlan.self).first!.asJSON())
	...
	func testCarePlanRealm6(instance: FireKit.CarePlan) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runCarePlan6(realm.objects(FireKit.CarePlan.self).first!.asJSON())
	...
	func testCarePlanRealm7(instance: FireKit.CarePlan) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runCarePlan7(realm.objects(FireKit.CarePlan.self).first!.asJSON())
	...
	func testCarePlanRealm8(instance: FireKit.CarePlan) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runCarePlan8(realm.objects(FireKit.CarePlan.self).first!.asJSON())
	...
	func testCarePlanRealm9(instance: FireKit.CarePlan) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runCarePlan9(realm.objects(FireKit.CarePlan.self).first!.asJSON())
	...
	func testCarePlanRealm10(instance: FireKit.CarePlan) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runCarePlan10(realm.objects(FireKit.CarePlan.self).first!.asJSON())
