...
extension Reference {
    ...
    public func resolve<T>(from realm: Realm) -> T? where T: Resource {
        ...
        switch parts.fhirType {
            case "Account":
                return self.realm?.objects(Account.self).filter("id = %@", parts.id).first as? T
            case "AllergyIntolerance":
                return self.realm?.objects(AllergyIntolerance.self).filter("id = %@", parts.id).first as? T
            case "Appointment":
                return self.realm?.objects(Appointment.self).filter("id = %@", parts.id).first as? T
            case "AppointmentResponse":
                return self.realm?.objects(AppointmentResponse.self).filter("id = %@", parts.id).first as? T
            case "AuditEvent":
                return self.realm?.objects(AuditEvent.self).filter("id = %@", parts.id).first as? T
            case "Basic":
                return self.realm?.objects(Basic.self).filter("id = %@", parts.id).first as? T
            case "Binary":
                return self.realm?.objects(Binary.self).filter("id = %@", parts.id).first as? T
            case "BodySite":
                return self.realm?.objects(BodySite.self).filter("id = %@", parts.id).first as? T
            case "Bundle":
                return self.realm?.objects(Bundle.self).filter("id = %@", parts.id).first as? T
            case "CarePlan":
                return self.realm?.objects(CarePlan.self).filter("id = %@", parts.id).first as? T
            case "Claim":
                return self.realm?.objects(Claim.self).filter("id = %@", parts.id).first as? T
            case "ClaimResponse":
                return self.realm?.objects(ClaimResponse.self).filter("id = %@", parts.id).first as? T
            case "ClinicalImpression":
                return self.realm?.objects(ClinicalImpression.self).filter("id = %@", parts.id).first as? T
            case "Communication":
                return self.realm?.objects(Communication.self).filter("id = %@", parts.id).first as? T
            case "CommunicationRequest":
                return self.realm?.objects(CommunicationRequest.self).filter("id = %@", parts.id).first as? T
            case "Composition":
                return self.realm?.objects(Composition.self).filter("id = %@", parts.id).first as? T
            case "ConceptMap":
                return self.realm?.objects(ConceptMap.self).filter("id = %@", parts.id).first as? T
            case "Condition":
                return self.realm?.objects(Condition.self).filter("id = %@", parts.id).first as? T
            case "Conformance":
                return self.realm?.objects(Conformance.self).filter("id = %@", parts.id).first as? T
            case "Contract":
                return self.realm?.objects(Contract.self).filter("id = %@", parts.id).first as? T
            case "Coverage":
                return self.realm?.objects(Coverage.self).filter("id = %@", parts.id).first as? T
            case "DataElement":
                return self.realm?.objects(DataElement.self).filter("id = %@", parts.id).first as? T
            case "DetectedIssue":
                return self.realm?.objects(DetectedIssue.self).filter("id = %@", parts.id).first as? T
            case "Device":
                return self.realm?.objects(Device.self).filter("id = %@", parts.id).first as? T
            case "DeviceComponent":
                return self.realm?.objects(DeviceComponent.self).filter("id = %@", parts.id).first as? T
            case "DeviceMetric":
                return self.realm?.objects(DeviceMetric.self).filter("id = %@", parts.id).first as? T
            case "DeviceUseRequest":
                return self.realm?.objects(DeviceUseRequest.self).filter("id = %@", parts.id).first as? T
            case "DeviceUseStatement":
                return self.realm?.objects(DeviceUseStatement.self).filter("id = %@", parts.id).first as? T
            case "DiagnosticOrder":
                return self.realm?.objects(DiagnosticOrder.self).filter("id = %@", parts.id).first as? T
            case "DiagnosticReport":
                return self.realm?.objects(DiagnosticReport.self).filter("id = %@", parts.id).first as? T
            case "DocumentManifest":
                return self.realm?.objects(DocumentManifest.self).filter("id = %@", parts.id).first as? T
            case "DocumentReference":
                return self.realm?.objects(DocumentReference.self).filter("id = %@", parts.id).first as? T
            case "DomainResource":
                return self.realm?.objects(DomainResource.self).filter("id = %@", parts.id).first as? T
            case "EligibilityRequest":
                return self.realm?.objects(EligibilityRequest.self).filter("id = %@", parts.id).first as? T
            case "EligibilityResponse":
                return self.realm?.objects(EligibilityResponse.self).filter("id = %@", parts.id).first as? T
            case "Encounter":
                return self.realm?.objects(Encounter.self).filter("id = %@", parts.id).first as? T
            case "EnrollmentRequest":
                return self.realm?.objects(EnrollmentRequest.self).filter("id = %@", parts.id).first as? T
            case "EnrollmentResponse":
                return self.realm?.objects(EnrollmentResponse.self).filter("id = %@", parts.id).first as? T
            case "EpisodeOfCare":
                return self.realm?.objects(EpisodeOfCare.self).filter("id = %@", parts.id).first as? T
            case "ExplanationOfBenefit":
                return self.realm?.objects(ExplanationOfBenefit.self).filter("id = %@", parts.id).first as? T
            case "FamilyMemberHistory":
                return self.realm?.objects(FamilyMemberHistory.self).filter("id = %@", parts.id).first as? T
            case "Flag":
                return self.realm?.objects(Flag.self).filter("id = %@", parts.id).first as? T
            case "Goal":
                return self.realm?.objects(Goal.self).filter("id = %@", parts.id).first as? T
            case "Group":
                return self.realm?.objects(Group.self).filter("id = %@", parts.id).first as? T
            case "HealthcareService":
                return self.realm?.objects(HealthcareService.self).filter("id = %@", parts.id).first as? T
            case "ImagingObjectSelection":
                return self.realm?.objects(ImagingObjectSelection.self).filter("id = %@", parts.id).first as? T
            case "ImagingStudy":
                return self.realm?.objects(ImagingStudy.self).filter("id = %@", parts.id).first as? T
            case "Immunization":
                return self.realm?.objects(Immunization.self).filter("id = %@", parts.id).first as? T
            case "ImmunizationRecommendation":
                return self.realm?.objects(ImmunizationRecommendation.self).filter("id = %@", parts.id).first as? T
            case "ImplementationGuide":
                return self.realm?.objects(ImplementationGuide.self).filter("id = %@", parts.id).first as? T
            case "List":
                return self.realm?.objects(List.self).filter("id = %@", parts.id).first as? T
            case "Location":
                return self.realm?.objects(Location.self).filter("id = %@", parts.id).first as? T
            case "Media":
                return self.realm?.objects(Media.self).filter("id = %@", parts.id).first as? T
            case "Medication":
                return self.realm?.objects(Medication.self).filter("id = %@", parts.id).first as? T
            case "MedicationAdministration":
                return self.realm?.objects(MedicationAdministration.self).filter("id = %@", parts.id).first as? T
            case "MedicationDispense":
                return self.realm?.objects(MedicationDispense.self).filter("id = %@", parts.id).first as? T
            case "MedicationOrder":
                return self.realm?.objects(MedicationOrder.self).filter("id = %@", parts.id).first as? T
            case "MedicationStatement":
                return self.realm?.objects(MedicationStatement.self).filter("id = %@", parts.id).first as? T
            case "MessageHeader":
                return self.realm?.objects(MessageHeader.self).filter("id = %@", parts.id).first as? T
            case "NamingSystem":
                return self.realm?.objects(NamingSystem.self).filter("id = %@", parts.id).first as? T
            case "NutritionOrder":
                return self.realm?.objects(NutritionOrder.self).filter("id = %@", parts.id).first as? T
            case "Observation":
                return self.realm?.objects(Observation.self).filter("id = %@", parts.id).first as? T
            case "OperationDefinition":
                return self.realm?.objects(OperationDefinition.self).filter("id = %@", parts.id).first as? T
            case "OperationOutcome":
                return self.realm?.objects(OperationOutcome.self).filter("id = %@", parts.id).first as? T
            case "Order":
                return self.realm?.objects(Order.self).filter("id = %@", parts.id).first as? T
            case "OrderResponse":
                return self.realm?.objects(OrderResponse.self).filter("id = %@", parts.id).first as? T
            case "Organization":
                return self.realm?.objects(Organization.self).filter("id = %@", parts.id).first as? T
            case "Parameters":
                return self.realm?.objects(Parameters.self).filter("id = %@", parts.id).first as? T
            case "Patient":
                return self.realm?.objects(Patient.self).filter("id = %@", parts.id).first as? T
            case "PaymentNotice":
                return self.realm?.objects(PaymentNotice.self).filter("id = %@", parts.id).first as? T
            case "PaymentReconciliation":
                return self.realm?.objects(PaymentReconciliation.self).filter("id = %@", parts.id).first as? T
            case "Person":
                return self.realm?.objects(Person.self).filter("id = %@", parts.id).first as? T
            case "Practitioner":
                return self.realm?.objects(Practitioner.self).filter("id = %@", parts.id).first as? T
            case "Procedure":
                return self.realm?.objects(Procedure.self).filter("id = %@", parts.id).first as? T
            case "ProcedureRequest":
                return self.realm?.objects(ProcedureRequest.self).filter("id = %@", parts.id).first as? T
            case "ProcessRequest":
                return self.realm?.objects(ProcessRequest.self).filter("id = %@", parts.id).first as? T
            case "ProcessResponse":
                return self.realm?.objects(ProcessResponse.self).filter("id = %@", parts.id).first as? T
            case "Provenance":
                return self.realm?.objects(Provenance.self).filter("id = %@", parts.id).first as? T
            case "Questionnaire":
                return self.realm?.objects(Questionnaire.self).filter("id = %@", parts.id).first as? T
            case "QuestionnaireResponse":
                return self.realm?.objects(QuestionnaireResponse.self).filter("id = %@", parts.id).first as? T
            case "ReferralRequest":
                return self.realm?.objects(ReferralRequest.self).filter("id = %@", parts.id).first as? T
            case "RelatedPerson":
                return self.realm?.objects(RelatedPerson.self).filter("id = %@", parts.id).first as? T
            case "RiskAssessment":
                return self.realm?.objects(RiskAssessment.self).filter("id = %@", parts.id).first as? T
            case "Schedule":
                return self.realm?.objects(Schedule.self).filter("id = %@", parts.id).first as? T
            case "SearchParameter":
                return self.realm?.objects(SearchParameter.self).filter("id = %@", parts.id).first as? T
            case "Slot":
                return self.realm?.objects(Slot.self).filter("id = %@", parts.id).first as? T
            case "Specimen":
                return self.realm?.objects(Specimen.self).filter("id = %@", parts.id).first as? T
            case "StructureDefinition":
                return self.realm?.objects(StructureDefinition.self).filter("id = %@", parts.id).first as? T
            case "Subscription":
                return self.realm?.objects(Subscription.self).filter("id = %@", parts.id).first as? T
            case "Substance":
                return self.realm?.objects(Substance.self).filter("id = %@", parts.id).first as? T
            case "SupplyDelivery":
                return self.realm?.objects(SupplyDelivery.self).filter("id = %@", parts.id).first as? T
            case "SupplyRequest":
                return self.realm?.objects(SupplyRequest.self).filter("id = %@", parts.id).first as? T
            case "TestScript":
                return self.realm?.objects(TestScript.self).filter("id = %@", parts.id).first as? T
            case "ValueSet":
                return self.realm?.objects(ValueSet.self).filter("id = %@", parts.id).first as? T
            case "VisionPrescription":
                return self.realm?.objects(VisionPrescription.self).filter("id = %@", parts.id).first as? T
            default:
