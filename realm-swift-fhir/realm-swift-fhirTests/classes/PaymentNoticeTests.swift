...
class PaymentNoticeTests: XCTestCase, RealmPersistenceTesting {    
	...
	func testPaymentNotice1() {		
		...
		do {
			...
			let copy = instance!.copy() as? RealmSwiftFHIR.PaymentNotice
    ...
    func testPaymentNotice1RealmPK() {        
        do {
            let instance: RealmSwiftFHIR.PaymentNotice = try runPaymentNotice1()
            let copy = (instance.copy() as! RealmSwiftFHIR.PaymentNotice)
	...
	func testPaymentNoticeRealm1(instance: RealmSwiftFHIR.PaymentNotice) {
        ...
        let newInst = RealmSwiftFHIR.PaymentNotice()
        try! realm.write { realm.add(newInst) }
        
        // first time updating it should inflate children resources/elements which don't exist
        var existing = realm.object(ofType: RealmSwiftFHIR.PaymentNotice.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runPaymentNotice1(existing.asJSON())
        
        // second time updating it will overwrite values of child resources/elements, but maintain keys
        // TODO: Find a way to actually test this instead of breakpoints and eyeballing it.
        existing = realm.object(ofType: RealmSwiftFHIR.PaymentNotice.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runPaymentNotice1(existing.asJSON())
        ...
        XCTAssertEqual(0, realm.objects(RealmSwiftFHIR.PaymentNotice.self).count)
	}
