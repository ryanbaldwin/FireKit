...
class HealthcareServiceTests: XCTestCase, RealmPersistenceTesting {    
	...
	func testHealthcareService1() {		
		...
		do {
			...
			let copy = instance!.copy() as? RealmSwiftFHIR.HealthcareService
    ...
    func testHealthcareService1RealmPK() {        
        do {
            let instance: RealmSwiftFHIR.HealthcareService = try runHealthcareService1()
            let copy = (instance.copy() as! RealmSwiftFHIR.HealthcareService)
	...
	func testHealthcareServiceRealm1(instance: RealmSwiftFHIR.HealthcareService) {
        ...
        let newInst = RealmSwiftFHIR.HealthcareService()
        try! realm.write { realm.add(newInst) }
        
        // first time updating it should inflate children resources/elements which don't exist
        var existing = realm.object(ofType: RealmSwiftFHIR.HealthcareService.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runHealthcareService1(existing.asJSON())
        
        // second time updating it will overwrite values of child resources/elements, but maintain keys
        // TODO: Find a way to actually test this instead of breakpoints and eyeballing it.
        existing = realm.object(ofType: RealmSwiftFHIR.HealthcareService.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runHealthcareService1(existing.asJSON())
        ...
        XCTAssertEqual(0, realm.objects(RealmSwiftFHIR.HealthcareService.self).count)
	}
