...
class EligibilityRequestTests: XCTestCase, RealmPersistenceTesting {    
	...
	func testEligibilityRequest1() {		
		...
		do {
			...
			let copy = instance!.copy() as? RealmSwiftFHIR.EligibilityRequest
    ...
    func testEligibilityRequest1RealmPK() {        
        do {
            let instance: RealmSwiftFHIR.EligibilityRequest = try runEligibilityRequest1()
            let copy = (instance.copy() as! RealmSwiftFHIR.EligibilityRequest)
	...
	func testEligibilityRequestRealm1(instance: RealmSwiftFHIR.EligibilityRequest) {
        ...
        let newInst = RealmSwiftFHIR.EligibilityRequest()
        try! realm.write { realm.add(newInst) }
        
        // first time updating it should inflate children resources/elements which don't exist
        var existing = realm.object(ofType: RealmSwiftFHIR.EligibilityRequest.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runEligibilityRequest1(existing.asJSON())
        
        // second time updating it will overwrite values of child resources/elements, but maintain keys
        // TODO: Find a way to actually test this instead of breakpoints and eyeballing it.
        existing = realm.object(ofType: RealmSwiftFHIR.EligibilityRequest.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runEligibilityRequest1(existing.asJSON())
        ...
        XCTAssertEqual(0, realm.objects(RealmSwiftFHIR.EligibilityRequest.self).count)
	}
