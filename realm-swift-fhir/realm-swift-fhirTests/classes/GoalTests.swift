...
class GoalTests: XCTestCase, RealmPersistenceTesting {    
	...
	func testGoal1() {		
		...
		do {
			...
			let copy = instance!.copy() as? RealmSwiftFHIR.Goal
    ...
    func testGoal1RealmPK() {        
        do {
            let instance: RealmSwiftFHIR.Goal = try runGoal1()
            let copy = (instance.copy() as! RealmSwiftFHIR.Goal)
	...
	func testGoalRealm1(instance: RealmSwiftFHIR.Goal) {
        ...
        let newInst = RealmSwiftFHIR.Goal()
        try! realm.write { realm.add(newInst) }
        
        // first time updating it should inflate children resources/elements which don't exist
        var existing = realm.object(ofType: RealmSwiftFHIR.Goal.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runGoal1(existing.asJSON())
        
        // second time updating it will overwrite values of child resources/elements, but maintain keys
        // TODO: Find a way to actually test this instead of breakpoints and eyeballing it.
        existing = realm.object(ofType: RealmSwiftFHIR.Goal.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runGoal1(existing.asJSON())
        ...
        XCTAssertEqual(0, realm.objects(RealmSwiftFHIR.Goal.self).count)
	}
