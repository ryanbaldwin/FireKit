...
class TestScriptTests: XCTestCase, RealmPersistenceTesting {    
	...
	func instantiateFrom(filename: String) throws -> RealmSwiftFHIR.TestScript {
		return instantiateFrom(json: try readJSONFile(filename))
	}
	
	func instantiateFrom(json: FHIRJSON) -> RealmSwiftFHIR.TestScript {
		let instance = RealmSwiftFHIR.TestScript(json: json)
		XCTAssertNotNil(instance, "Must have instantiated a test instance")
		return instance
	}
	
	func testTestScript1() {		
		var instance: RealmSwiftFHIR.TestScript?
		do {
			instance = try runTestScript1()
			try runTestScript1(instance!.asJSON()) 		
			let copy = instance!.copy() as? RealmSwiftFHIR.TestScript
			XCTAssertNotNil(copy)
			try runTestScript1(copy!.asJSON())            
		}
		catch {
    ...
    func testTestScript1RealmPK() {        
        do {
            let instance: RealmSwiftFHIR.TestScript = try runTestScript1()
            let copy = (instance.copy() as! RealmSwiftFHIR.TestScript)
	...
	func testTestScriptRealm1(instance: RealmSwiftFHIR.TestScript) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runTestScript1(realm.objects(RealmSwiftFHIR.TestScript.self).first!.asJSON())
        
        // ensure we can update it.
        try! realm.write { instance.implicitRules = "Rule #1" }
        XCTAssertEqual(1, realm.objects(RealmSwiftFHIR.TestScript.self).count)
        XCTAssertEqual("Rule #1", realm.objects(RealmSwiftFHIR.TestScript.self).first!.implicitRules)
        
        // create a new instance with default key, save it, then populate it from instance JSON. 
        // PK should persist and not be overwritten.
        let newInst = RealmSwiftFHIR.TestScript()
        try! realm.write { realm.add(newInst) }
        
        // first time updating it should inflate children resources/elements which don't exist
        var existing = realm.object(ofType: RealmSwiftFHIR.TestScript.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runTestScript1(existing.asJSON())
        
        // second time updating it will overwrite values of child resources/elements, but maintain keys
        // TODO: Find a way to actually test this instead of breakpoints and eyeballing it.
        existing = realm.object(ofType: RealmSwiftFHIR.TestScript.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runTestScript1(existing.asJSON())
        ...
        XCTAssertEqual(0, realm.objects(RealmSwiftFHIR.TestScript.self).count)
	}
	
	@discardableResult
	func runTestScript1(_ json: FHIRJSON? = nil) throws -> RealmSwiftFHIR.TestScript {
	...
	func testTestScript2() {		
		...
		do {
			...
			let copy = instance!.copy() as? RealmSwiftFHIR.TestScript
			XCTAssertNotNil(copy)
			try runTestScript2(copy!.asJSON())            
		}
		catch {
			XCTAssertTrue(false, "Must instantiate and test TestScript successfully, but threw")
		}
    ...
    func testTestScript2RealmPK() {        
        do {
            let instance: RealmSwiftFHIR.TestScript = try runTestScript2()
            let copy = (instance.copy() as! RealmSwiftFHIR.TestScript)
	...
	func testTestScriptRealm2(instance: RealmSwiftFHIR.TestScript) {
		// ensure we can write the instance, then fetch it, serialize it to JSON, then deserialize that JSON 
        // and ensure it passes the all the same tests.
		try! realm.write {
                realm.add(instance)
            }
        try! runTestScript2(realm.objects(RealmSwiftFHIR.TestScript.self).first!.asJSON())
	...
	@discardableResult
	func runTestScript2(_ json: FHIRJSON? = nil) throws -> RealmSwiftFHIR.TestScript {
