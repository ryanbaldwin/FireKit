...
class ClaimResponseTests: XCTestCase, RealmPersistenceTesting {    
	...
	func testClaimResponse1() {		
		...
		do {
			...
			let copy = instance!.copy() as? RealmSwiftFHIR.ClaimResponse
    ...
    func testClaimResponse1RealmPK() {        
        do {
            let instance: RealmSwiftFHIR.ClaimResponse = try runClaimResponse1()
            let copy = (instance.copy() as! RealmSwiftFHIR.ClaimResponse)
	...
	func testClaimResponseRealm1(instance: RealmSwiftFHIR.ClaimResponse) {
        ...
        let newInst = RealmSwiftFHIR.ClaimResponse()
        try! realm.write { realm.add(newInst) }
        
        // first time updating it should inflate children resources/elements which don't exist
        var existing = realm.object(ofType: RealmSwiftFHIR.ClaimResponse.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runClaimResponse1(existing.asJSON())
        
        // second time updating it will overwrite values of child resources/elements, but maintain keys
        // TODO: Find a way to actually test this instead of breakpoints and eyeballing it.
        existing = realm.object(ofType: RealmSwiftFHIR.ClaimResponse.self, forPrimaryKey: newInst.pk)!
        try! realm.write{ _ = existing.populate(from: instance.asJSON()) }
        try! runClaimResponse1(existing.asJSON())
        ...
        XCTAssertEqual(0, realm.objects(RealmSwiftFHIR.ClaimResponse.self).count)
	}
